...
- [x] En locale FR, le texte du bouton défi du jour est lisible — le ratio de contraste texte/fond est conforme WCAG 2.1 AA (minimum 4.5:1 pour un texte normal)
- [x] En locale EN, le comportement et le rendu du bouton restent identiques à l'état actuel (aucune régression)
- [x] Le fix est vérifié dans les deux thèmes si l'app supporte un mode sombre, sinon en mode clair uniquement
...
## Validation QA — Halim
Validé le 2026-03-20. `dailyButtonTextDisabled.color = '#475569'` confirmé dans HomeScreen.tsx (ligne 861). Ratio WCAG AA 4.5:1 garanti sur fond `#E2E8F0`. Styles `dailyButtonText` et `dailyButtonTextCompleted` inchangés. tsc sans erreur, lint sans erreur (warnings non bloquants).
